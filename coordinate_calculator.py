...
class CoordinateCalculator:
    def __init__(self, depth_scale=None):
        ...
        self.use_undistortion = True  # Default to ON
        self.use_color_matrix_for_3d = True  # Default to Color
        print(f"🔧 Coordinate calculator initialized - depth_scale: {depth_scale}")
    ...
    def get_3d_coordinates(self, bbox, depth_image, K_depth, dist_coeffs_depth, K_color, dist_coeffs_color):
        ...
        if 0 <= center_x < depth_image.shape[1] and 0 <= center_y < depth_image.shape[0]:
            ...
            if depth_raw > 0:
                ...
                if self.use_undistortion:
                    ...
                    pixel_info = f"Pixel: ({center_x}, {center_y}) → Undistorted: ({final_x:.1f}, {final_y:.1f})"
                else:
                    final_x = center_x
                    final_y = center_y
                    pixel_info = f"Pixel: ({center_x}, {center_y})"
                
                # Select matrix for 3D calculation
                if self.use_color_matrix_for_3d:
                    calc_cx = K_color[0, 2]
                    calc_cy = K_color[1, 2]
                    calc_fx = K_color[0, 0]
                    calc_fy = K_color[1, 1]
                    matrix_info = f"Color matrix: fx={calc_fx:.1f}, fy={calc_fy:.1f}, cx={calc_cx:.1f}, cy={calc_cy:.1f}"
                else:
                    calc_cx = cx
                    calc_cy = cy
                    calc_fx = fx
                    calc_fy = fy
                    matrix_info = f"Depth matrix: fx={calc_fx:.1f}, fy={calc_fy:.1f}, cx={calc_cx:.1f}, cy={calc_cy:.1f}"
                ...
                print(f"📍 3D coordinates: ({x_3d*1000:.1f}, {y_3d*1000:.1f}, {z_3d*1000:.1f})mm")
                print(f"   📐 {pixel_info}")
                print(f"   🎯 {matrix_info}")
        ...
        print(f"❌ Invalid pixel coordinates: ({center_x}, {center_y})")
    ...
    def get_camera_matrices(self, color_intrinsics, depth_intrinsics):
        ...
        print(f"🔧 Camera matrices generated:")
        ...
        print(f"   Depth distortion coefficients: {dist_coeffs_depth}")
        print(f"   Color distortion coefficients: {dist_coeffs_color}")
